          ...
          >
            ...
            <div className="shrink grow flex flex-col items-start truncate">
              ...
              {file.errors.length > 0 ? (
                  ...
                  {file.errors
                    .map((e) =>
                      e.message.startsWith('File is larger than')
                        ? `El archivo pesa mas de ${formatBytes(maxFileSize, 2)} (Tamaño: ${formatBytes(file.size, 2)})`
                        : e.message
                    )
                    .join(', ')}
                </p>
                ...
                <p className="text-xs text-destructive">Hubo un error al cargar el archivo: {fileError.message}</p>
              ) : isSuccessfullyUploaded ? (
                <p className="text-xs text-primary">Subido correctamente</p>
              ) : (
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn('flex flex-col items-center gap-y-2', className)}>
      ...
      <p className="text-sm">
        Cargar{!!maxFiles && maxFiles > 1 ? ` ${maxFiles}` : ''} archivo
      ...
      <div className="flex flex-col items-center gap-y-1">
        <p className="text-xs text-muted-foreground">
          ...
          >
            selecciona los {maxFiles === 1 ? `archivo` : 'archivos'}
          </a>{' '}
          
        </p>
        {maxFileSize !== Number.POSITIVE_INFINITY && (
          <p className="text-xs text-muted-foreground">
            Tamaño maximo: {formatBytes(maxFileSize, 2)}
          </p>
        )}
      </div>
    </div>
